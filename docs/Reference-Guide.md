...
- `frontend/streamlit_app/components/autotriage_view.py`: Auto-triage visualization component implementing DataUnderstanding_v2.md frontend specification with enhanced suspicion rankings (contribution breakdowns, stacked bar charts, navigation links), PCA analysis (variance overview with bar+line charts, narrative feed, loadings CSV download, component comparison radar charts), change-point detection (time-series with markers, segment summary tables, context chips for cluster shifts), and cluster deep-dive (size distribution, medoid samples with pagination, ANOVA feature differences, tree-based importance, temporal evolution charts). Supports both enhanced and legacy payload formats with responsive CSS injection.

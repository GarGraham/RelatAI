...
- `frontend/streamlit_app/components/__init__.py`: Reusable Streamlit component exports including column selector, filter panel, mode selector, and preview table components.
...
- `frontend/streamlit_app/utils/session_state.py`: Session state management utilities providing initialization helpers, safe state access, context loading, dataset status checks, configuration management helpers (has_configuration, get_configuration, update_configuration, reset_configuration), analysis state resets, and debug display for development.

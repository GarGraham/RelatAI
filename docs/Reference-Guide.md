...
- `docs/MILESTONE9_PHASE3_COMPLETION.md`: Completion report for Phase 3 (Analysis Execution & Results Display) documenting all deliverables (confidence flag components, visualization components for correlation/multivariate/auto-triage, AI summary component, analysis page with mode-specific tabs and export functionality, enhanced session state management), success metrics achieved, technical implementation details, visual design quality, code organization assessment, testing performed, known limitations, and next steps for Phase 4.
- `docs/MILESTONE9_PHASE4_COMPLETION.md`: Completion report for Phase 4 (Polish & Advanced Features) documenting all deliverables (export utilities with multi-format support, audit trail viewer components and dedicated page, advanced configuration options for preprocessing/performance/sensitivity, comprehensive help system with statistical documentation, error handling with category-based routing, comprehensive validation system), success metrics achieved, technical implementation details, code quality and UX assessment, testing performed, known limitations, and next steps for Phase 5.
...
- `backend/relat_ai/core/autotriage_models.py`: Typed Pydantic models for auto-triage explainability payloads including SuspicionItemModel (contribution breakdowns), ClusterProfileModel (statistical profiling), PCAExplainModel (narrative explanations), ChangePointReportModel (segment summaries), and EvidenceBundle (aggregated narratives).
...
- `backend/relat_ai/services/analysis/auto_triage.py`: Auto-triage analysis pipeline combining PCA loadings, change-point detection (CUSUM, PELT algorithms), clustering (K-Means, Hierarchical), residual forensics, and suspicion ranking for unsupervised anomaly triage during quality events. Enhanced with SignalVector for normalized signal contributions, compute_cluster_profile() for ANOVA-based profiling, PCA narrative generation, and contextual change-point reporting with segment summaries.
...
- `frontend/streamlit_app/pages/2_⚙️_Configuration.py`: Configuration page providing column selection with search/filtering, dynamic filter builder with type-aware inputs, analysis mode selector with mode-specific parameters, live configuration preview, template management (save/load/delete functionality), and collapsible Advanced Settings section with preprocessing controls (6 imputation strategies, 4 outlier detection methods, 5 scaling methods, 2 power transforms), performance settings (data sampling with random/stratified options, parallel processing with worker count control, result caching), and sensitivity thresholds (alpha level slider, correlation minimum, minimum effect size, minimum sample size).
- `frontend/streamlit_app/pages/3_🔬_Analysis.py`: Analysis execution and results display page with run controls, progress tracking, mode-specific result tabs (correlation/multivariate/auto-triage), confidence flag displays, AI-generated summaries, and enhanced export options providing 3-tab interface (Quick Export for single-file JSON/CSV/TXT metadata, Complete Package for multi-file bundles with all formats and package contents preview, Data Only for raw CSV tables and compact JSON) with dynamic timestamp naming and export tips expandable section.
...
- `frontend/streamlit_app/components/audit_viewer.py`: Audit trail visualization components providing timeline view with expandable entry details, filtering by dataset ID, action types, and date ranges, summary metrics (total entries, unique datasets, action types, time span), human-readable relative timestamps, and action-specific icons for comprehensive preprocessing transparency.
- `frontend/streamlit_app/components/help_system.py`: Contextual help system providing inline help icons with tooltips, detailed statistical method documentation (Pearson, Spearman, Kendall, Regression, ANOVA, PCA, Change-point, Clustering) with assumptions and interpretation guides, statistical interpretation references (correlation strength, p-values, R², VIF, confidence intervals) with threshold guidelines, topic-specific best practices checklists (data quality, correlation analysis, multivariate analysis, auto-triage), and sidebar quick help widget for onboarding support.
- `frontend/streamlit_app/pages/4_📋_Audit_Trail.py`: Dedicated audit trail page with scope selection (current dataset vs all datasets), comprehensive filtering controls (dataset ID search, action type multi-select, date range presets including Last 24h/7d/30d/Custom), summary metrics display, timeline view with expandable entry details showing impact metrics and user tracking, CSV export functionality, and extensive help documentation covering use cases and best practices for preprocessing transparency investigation.
- `frontend/streamlit_app/utils/__init__.py`: Utility package exports for API client, session state management with configuration helpers, error handling functions, export utilities, validators, and error handlers.
...
- `frontend/streamlit_app/utils/export_utils.py`: Comprehensive export utilities supporting multi-format result export including JSON (full results with metadata timestamp), CSV (mode-specific correlation tables, multivariate coefficient tables, auto-triage suspicion rankings), TXT (formatted metadata), complete export packages (multi-file bundles with all formats), visualization data extraction, and audit log CSV export with ISO timestamp sorting.
- `frontend/streamlit_app/utils/error_handlers.py`: User-friendly error handling system providing UserFriendlyError exception class with categorization (NETWORK, VALIDATION, NOT_FOUND, SERVER, TIMEOUT, PERMISSION, DATA, UNKNOWN), format_error_message() converting standard exceptions to user-friendly format with category-specific suggestions, display_error() with Streamlit integration showing categorized icons and expandable technical details, handle_error() wrapper for function execution, validate_and_execute() pattern for pre-execution validation, and create_retry_handler() with exponential backoff support.

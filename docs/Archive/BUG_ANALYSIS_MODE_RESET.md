...
1. Open **Configuration** (rontend/streamlit_app/pages/2__Configuration.py).
...
The users screenshot confirms the Auto-Triage tab was active, yet no success banner ( Auto-Triage mode active) was displayedmeaning the underlying state never changed.

---

## Impact
- **Functional**: Auto-Triage analysis never runs because the configuration still points to the default correlation mode.
- **User Experience**: UI provides misleading feedbacktab activation changes layout but not the saved configuration, so users believe their choice was honored.
...
### Summary
ender_mode_selector() stores the chosen analysis mode exclusively through the "Select  Mode" buttons. Clicking a tab *only* reveals descriptive contentit does **not** mutate st.session_state.selected_mode. Therefore, when Apply is clicked without pressing the button, _apply_configuration() sends the previous mode (correlation) to the backend, which persists it. The Analysis page then correctly reports correlation.

### Detailed Trace
- **File**: rontend/streamlit_app/components/mode_selector.py
- **Critical snippet**:
  `python
  with tab3:
...
- _apply_configuration() passes nalysis_mode=new_mode where 
ew_mode is the return value of ender_mode_selector()	he unchanged previous mode.
...
3. Regression-test all three modes to confirm the payload carries the correct nalysis_mode.
